var Life = (function () {
    function Life(rs, cs, speed, th) {
        ...
        this.render = function () {
            ...
            _this.frameCount += 1;
            if (_this.frameCount >= _this.updateSpeed) {
                _this.update();
                _this.ctx.clearRect(0, 0, _this.boardCanvas.width, _this.boardCanvas.height);
                var h = _this.boardCanvas.height / _this.rows;
                var w = _this.boardCanvas.width / _this.cols;
                for (var i = 0; i < _this.rows; i++) {
                    for (var j = 0; j < _this.cols; j++) {
                        if (_this.board[i][j] > 0) {
                            _this.ctx.fillRect(j * w, i * h, w, h);
                        }
                    }
                }
                _this.frameCount = 0;
    ...
    Life.prototype.replace = function (bd) {
        this.rows = bd.length;
        this.cols = bd[0].length;
        this.board = bd;
    };
    Life.prototype.randomize = function (threshold) {
